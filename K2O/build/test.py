...
def wait_for_db_bench():
    ...
    while True:
        # Check if any db_bench process is running
...
def save_iops_file(log_file, exp_dir, tag):
    ...
    # Find line starting with tag
    for line in content.split('\n'):
        if line.startswith(tag):
            # Extract IOPS value (number before "ops/sec")
...
def collect_trace_breakdown(exp_dir, bench_names, use_li, suffix):
    ...
    for bench in bench_names:
        ...
        src_candidates = [
            os.path.join(exp_dir, src_name),  # db_bench writes directly to latency_stats_dir
            src_name,  # fallback: current working directory
        ]
...
def run_test(use_li, test_type, ycsb_type='x', key_size=None, value_size=None, exp_log_path=None, dataset=None):
    ...
    #sleep 20 seconds to wait for system stable
    ...
    if test_type == "random":
        ...
        collect_trace_breakdown(exp_dir, ["fillrandom", "readrandom"], use_li, value_size)
        save_io_count_breakdown(exp_dir, "fillrandom", use_li, value_size, log_path)
        save_io_count_breakdown(exp_dir, "readrandom", use_li, value_size, log_path)
    elif test_type == "ycsb":
        save_iops_file(log_path, exp_dir, "ycsb")
        collect_trace_breakdown(exp_dir, ["ycsb"], use_li, value_size)
        save_io_count_breakdown(exp_dir, "ycsb", use_li, value_size, log_path)
...
def main():
    ...
    #             exp_log_path=f"{EXP_ROOT}/k{DEFAULT_KEY_SIZE}_v{DEFAULT_VALUE_SIZE}/random_0/{'li' if li else 'no_li'}/")
