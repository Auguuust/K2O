...
struct BlockBasedTableOptions {
  ...
  // When enabled, the table builder will use a learned linear model to predict
  // byte offsets and create blocks such that the model's predictions are always
  // correct. This follows the approach from "Learned Indexes for a Google-scale
  // Disk-based Database" (https://arxiv.org/pdf/2012.12501)
  //
  // Benefits:
  // - Significantly smaller index size (model + block pointers vs full B-tree)
  // - Fewer disk reads for index blocks
  // - Better cache efficiency
  //
  // Note: This changes how blocks are constructed - blocks are created based on
  // model predictions rather than filling to a fixed size, resulting in slightly
  // variable block sizes but guaranteed model accuracy.
  bool enable_learned_index = false;

  // Target average block size for learned index.
  // The learned model will predict byte offsets and divide them by this value
  // to determine block numbers. Actual block sizes may vary around this target.
  // Default: same as block_size
  uint64_t learned_index_target_block_size = 4 * 1024;

  // Minimum number of keys required to train the learned index.
  // If fewer keys are present, falls back to standard block construction.
  uint32_t learned_index_min_training_keys = 100;

  // If true, trust learned index predictions and do NOT fall back to the
  // traditional index on misses. This can improve performance for tests but
  // may return false negatives if a prediction misses.
  bool learned_index_strict_mode = false;

  // Segment size (number of keys per segment) for two-level learned index.
  // Larger values mean fewer segments (coarser model, less accurate).
  // Default: 1,000,000 keys per segment.
  uint64_t learned_index_segment_keys = 1000000;
