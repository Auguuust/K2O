...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (may_match) {
    ...
    bool learned_fallback_needed = false;

    // Fast path: use learned index for direct data block lookup if available.
    if (rep_->table_options.enable_learned_index) {
      do {
        LocalOp index_op_timer;
        index_op_timer.begin();
        uint64_t file_num = 0;
        if (rep_->file) {
          file_num = TableFileNameToNumber(rep_->file->file_name());
        }
        if (file_num == 0 && rep_->table_properties &&
            rep_->table_properties->orig_file_number > 0) {
          file_num = rep_->table_properties->orig_file_number;
        }

        if (file_num == 0) {
          LearnedIndexManager::GetInstance().RecordLearnedFallback();
          break;
        }

        const LearnedIndexEntry* li_entry =
            LearnedIndexManager::GetInstance().GetLearnedIndex(file_num);
        if (!li_entry || !li_entry->learned_index || !li_entry->block_mapper) {
          LearnedIndexManager::GetInstance().RecordLearnedFallback();
          break;
        }
        ...
        uint32_t block_id = li_entry->learned_index->PredictBlock(user_key);
        index_op_timer.record_phase(0);
        if (block_id >= li_entry->block_mapper->GetNumBlocks()) {
          block_id = li_entry->block_mapper->GetNumBlocks() == 0
                         ? 0
                         : li_entry->block_mapper->GetNumBlocks() - 1;
        }
        auto loc = li_entry->block_mapper->GetBlockLocation(block_id);

        if (loc.size == 0) {
          learned_fallback_needed = true;
          break;
        }

        BlockHandle handle;
        handle.set_offset(loc.offset);
        handle.set_size(loc.size);
        ...
        NewDataBlockIterator<DataBlockIter>(
            read_options, handle, &biter, BlockType::kData, get_context,
        ...
        if (no_io && biter.status().IsIncomplete()) {
          ...
          LearnedIndexManager::GetInstance().RecordLearnedRead(
              false /* hit */, false /* fallback */);
          return biter.status();
        }
        if (!biter.status().ok()) {
          data_op_timer.commit(kDataBlockTime);
    ...
    // Optional strict mode: trust learned index and do not fall back to
    // traditional index if a learned lookup was attempted.
    if (learned_attempted && rep_->table_options.learned_index_strict_mode) {
      if (learned_fallback_needed) {
        LearnedIndexManager::GetInstance().RecordLearnedRead(
            false /* hit */, false /* fallback */);
      }
      return Status::OK();
    ...
    if (learned_attempted && learned_fallback_needed) {
      bool found = get_context->State() != GetContext::GetState::kNotFound;
      if (s.ok() && found) {
        LearnedIndexManager::GetInstance().RecordLearnedRead(
            false /* hit */, true /* fallback */);
      } else if (s.ok() && !found) {
        LearnedIndexManager::GetInstance().RecordLearnedFallback();
      } else {
        LearnedIndexManager::GetInstance().RecordLearnedRead(
