...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (IsValueType(value_type)) {
    ...
    auto policy_says_flush = r->flush_block_policy->Update(key, value);
    // Call virtual hook to let subclass override flush decision
    auto should_flush = ShouldFlushBeforeAdd(key, value, policy_says_flush);
    if (should_flush) {
      LocalOp flush_timer;
      flush_timer.begin();
...
void BlockBasedTableBuilder::WriteBlock(const Slice& uncompressed_block_data,
  ...
  if (is_data_block) {
    ...
    // Notify subclass of data block write
    OnDataBlockWritten(handle->offset(), static_cast<uint32_t>(handle->size()));
  }
  write_block_timer.record_phase();
