...
class BlockBasedTableBuilder : public TableBuilder {
 ...
 private:
  ...
  // REQUIRES: Finish(), Abandon() have not been called
  // NOTE: Changed to protected to allow LearnedBlockBasedTableBuilder to
  //       control block boundaries based on learned index predictions
 protected:
  void Flush();
  
  // Virtual hook to let subclass control flush decision
  // Called before adding each key. If returns true, flush before add.
  // Default behavior delegates to the flush_block_policy.
  // @param key: The key being added
  // @param value: The value being added  
  // @param policy_says_flush: What the default flush policy would return
  // @return: true if should flush before adding this key
  virtual bool ShouldFlushBeforeAdd(const Slice& key, const Slice& value, 
                                     bool policy_says_flush) {
    (void)key;
    (void)value;
    return policy_says_flush;  // Default: use the policy's decision
  }
  
  // Virtual callback invoked after a data block is written
  // Subclasses can override this to track block locations
  virtual void OnDataBlockWritten(uint64_t offset, uint32_t size) {
