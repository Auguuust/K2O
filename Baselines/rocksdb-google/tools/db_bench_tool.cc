...
DEFINE_string(
    ...
    "\treadmissinginrange -- read N missing keys between existing keys\n"
...
// Learned Index Options (Google's approach)
DEFINE_bool(enable_learned_index, false,
            "Enable learned index to guide block boundary decisions. "
            "Implements Google's approach from 'Learned Indexes for a "
            "Google-scale Disk-based Database' where the model defines "
            "block boundaries to ensure zero prediction error.");

DEFINE_uint64(learned_index_target_block_size, 4096,
              "Target average block size for learned index in bytes. "
              "The learned model will predict byte offsets and divide by "
              "this value to determine block numbers.");

DEFINE_uint32(learned_index_min_training_keys, 100,
              "Minimum number of keys required to train the learned index. "
              "If fewer keys are present, falls back to standard block "
              "construction.");

DEFINE_bool(learned_index_strict_mode, false,
            "If true, trust learned index predictions and do NOT fall back to "
            "traditional index on misses. This may return false negatives.");

DEFINE_uint64(learned_index_segment_keys, 1000000,
              "Number of keys per segment for two-level learned index. "
              "Larger values mean fewer segments (coarser model).");
...
class Stats {
 ...
 public:
  ...
  void SaveLatencyStats(const Slice& name) {
    ...
    // Also save PerfContext statistics if available
    SavePerfContextStats(name);
  }

  void SavePerfContextStats(const Slice& name) {
    if (FLAGS_latency_stats_dir.empty() || 
        FLAGS_perf_level <= ROCKSDB_NAMESPACE::PerfLevel::kDisable) {
      return;
    }
    
    std::string filename = FLAGS_latency_stats_dir + "/" + 
                          name.ToString() + "_perf_context.txt";
    
    std::unique_ptr<WritableFile> file;
    EnvOptions env_options;
    Status s = FLAGS_env->NewWritableFile(filename, &file, env_options);
    if (!s.ok()) {
      return;
    }

    // Write PerfContext detailed breakdown
    std::string header = "# PerfContext Statistics for " + name.ToString() + "\n\n";
    file->Append(header);
    
    // Get PerfContext string representation
    std::string perf_str = get_perf_context()->ToString(true);
    file->Append(perf_str);
    
    file->Sync();
    file->Close();
    
    fprintf(stdout, "Saved PerfContext stats to: %s\n", filename.c_str());
  }
};
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (name == "fillseqdeterministic" ||
        ...
        ++key_size_;
        method = &Benchmark::ReadRandom;
      } else if (name == "readmissinginrange") {
        if (key_size_ <= 8) {
          fprintf(stderr, "readmissinginrange requires key_size > 8\n");
          ErrorExit();
        }
        read_missing_in_range_ = true;
        method = &Benchmark::ReadRandom;
      ...
      if (method != nullptr) {
        ...
        std::string breakdown_json =
            name + (FLAGS_enable_learned_index ? "_li" : "_no_li") +
        ...
        fprintf(stdout, "Saved trace stats to: %s\n", breakdown_json.c_str());
      }
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    if (FLAGS_use_plain_table) {
      ...
      block_based_options.enable_learned_index = FLAGS_enable_learned_index;
      block_based_options.learned_index_target_block_size = 
          FLAGS_learned_index_target_block_size;
      block_based_options.learned_index_min_training_keys = 
          FLAGS_learned_index_min_training_keys;
      block_based_options.learned_index_strict_mode =
          FLAGS_learned_index_strict_mode;
      block_based_options.learned_index_segment_keys =
          FLAGS_learned_index_segment_keys;
...
// optimization, so skip the check of the max level.
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      GenerateKeyFromInt(key_rand, FLAGS_num, &key);
      if (read_missing_in_range_) {
        char* key_data = const_cast<char*>(key.data());
        key_data[key.size() - 1] = '1';
      }
      read++;
      std::string ts_ret;
      std::string* ts_ptr = nullptr;
