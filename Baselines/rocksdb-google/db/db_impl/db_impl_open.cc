  ...
  *dbptr = nullptr;
  ...
  if (s.ok()) {
    ...
    *dbptr = impl;
    ...
    // Initialize and load learned indexes from .li files
    LearnedIndexManager& li_manager = LearnedIndexManager::GetInstance();
    li_manager.Initialize(impl->env_, dbname);
    Status li_status = li_manager.LoadAllLearnedIndexes(dbname);
    if (!li_status.ok()) {
      ROCKS_LOG_WARN(impl->immutable_db_options_.info_log,
                     "Failed to load learned indexes: %s",
                     li_status.ToString().c_str());
    }
