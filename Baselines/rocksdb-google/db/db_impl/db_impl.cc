...
Status DBImpl::CloseHelper() {
  ...
  // Dump learned index stats on close
  {
    auto li_stats = LearnedIndexManager::GetInstance().GetStats();
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "LearnedIndex stats: training_failures=%lu "
                   "learned_reads=%lu learned_hits=%lu learned_misses=%lu "
                   "learned_fallbacks=%lu total_indexes=%zu "
                   "total_memory_bytes=%zu",
                   static_cast<unsigned long>(li_stats.training_failures),
                   static_cast<unsigned long>(li_stats.learned_reads),
                   static_cast<unsigned long>(li_stats.learned_hits),
                   static_cast<unsigned long>(li_stats.learned_misses),
                   static_cast<unsigned long>(li_stats.learned_fallbacks),
                   li_stats.total_indexes, li_stats.total_memory_bytes);
  }
