...
struct BlockBasedTableOptions {
  ...
  // ========== LeaderKV / DKTable Options ==========
  // Enable learned index for fast lookup (DKTable format)
  // When enabled, SST files will be written in DKTable format with
  // separated Value Area and Learn Area, and a learned index will be
  // trained for fast key lookup.
  // Default: false
  bool use_learned_index = false;

  // Error bound for PLR (Piecewise Linear Regression) algorithm
  // Controls the maximum prediction error allowed in the learned index.
  // Smaller values create more accurate but larger indexes.
  // Default: 32
  uint32_t learned_index_error_bound = 32;

  // Group size for learned index construction
  // Number of keys grouped together when building the learned index.
  // Affects the granularity of the index structure.
  // Default: 16
  uint32_t learned_index_group_size = 16;
};
