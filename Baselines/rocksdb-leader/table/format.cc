...
Status FooterBuilder::Build(uint64_t magic_number, uint32_t format_version,
                            ...
                            uint64_t dktable_value_area_size,
                            uint64_t dktable_learn_area_offset,
                            uint64_t dktable_learn_area_size) {
  ...
  if (format_version >= 6) {
    ...
    // DKTable: Store offsets in the 24 reserved bytes instead of zero padding
    // reserved1 (8 bytes) = dktable_value_area_size
    // reserved2 (8 bytes) = dktable_learn_area_offset
    // reserved3 (8 bytes) = dktable_learn_area_size
    EncodeFixed64(cur, dktable_value_area_size);
    cur += 8;
    EncodeFixed64(cur, dktable_learn_area_offset);
    cur += 8;
    EncodeFixed64(cur, dktable_learn_area_size);
    cur += 8;
    assert(cur == part3);
...
Status Footer::DecodeFrom(Slice input, uint64_t input_offset,
  ...
  if (format_version_ >= 6) {
    ...
    // DKTable: Read offsets from the 24 reserved bytes
    // reserved1 (8 bytes) = dktable_value_area_size
    // reserved2 (8 bytes) = dktable_learn_area_offset
    // reserved3 (8 bytes) = dktable_learn_area_size
    uint64_t reserved1 = 0, reserved2 = 0, reserved3 = 0;
    success = GetFixed64(&input, &reserved1);
    assert(success);
    success = GetFixed64(&input, &reserved2);
    assert(success);
    success = GetFixed64(&input, &reserved3);
    assert(success);
    
    // Store DKTable offsets (non-zero values indicate DKTable is present)
    dktable_value_area_size_ = reserved1;
    dktable_learn_area_offset_ = reserved2;
    dktable_learn_area_size_ = reserved3;
