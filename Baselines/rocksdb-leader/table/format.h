...
class Footer {
 public:
  ...
  // ========== DKTable Support ==========
  // DKTable stores Value Area at the beginning of SST, followed by Learn Area.
  // These offsets are stored in the reserved fields of format_version >= 6.
  bool has_dktable() const { return dktable_value_area_size_ > 0; }
  uint64_t dktable_value_area_size() const { return dktable_value_area_size_; }
  uint64_t dktable_learn_area_offset() const { return dktable_learn_area_offset_; }
  uint64_t dktable_learn_area_size() const { return dktable_learn_area_size_; }
  
  void set_dktable_info(uint64_t value_area_size, uint64_t learn_area_offset, 
                        uint64_t learn_area_size) {
    dktable_value_area_size_ = value_area_size;
    dktable_learn_area_offset_ = learn_area_offset;
    dktable_learn_area_size_ = learn_area_size;
  }
 ...
 private:
  ...
  // DKTable offsets - stored in reserved fields for format_version >= 6
  // reserved1 = dktable_value_area_size_
  // reserved2 = dktable_learn_area_offset_  
  // reserved3 = dktable_learn_area_size_
  uint64_t dktable_value_area_size_ = 0;
  uint64_t dktable_learn_area_offset_ = 0;
  uint64_t dktable_learn_area_size_ = 0;
};
...
class FooterBuilder {
 public:
  ...
  // * dktable_* parameters store DKTable offsets in reserved fields (format_version >= 6)
