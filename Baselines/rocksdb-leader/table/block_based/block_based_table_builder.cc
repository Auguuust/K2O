...
// LeaderKV / DKTable learned index support
...
struct BlockBasedTableBuilder::Rep {
  ...
  uint64_t li_build_begin_cycles = 0;
  ...
  // ========== DKTable / LeaderKV specific fields ==========
  // DKTable format in SST file:
  //   [Value Area]      - all values concatenated (start of file)
  //   [Learn Area]      - entries: [key + offset(4) + size(4)]
  //   [Filter Block]
  //   [MetaIndex Block]
  //   [Footer]          - contains learn_area_offset
  // .li file only stores: Leader learned index model (no key/value data)
  
  // Offset where Value Area starts (always 0 for DKTable)
  uint64_t value_area_offset = 0;
  // Offset where Learn Area starts (after Value Area)
  uint64_t learn_area_offset = 0;
  // Size of Learn Area
  uint64_t learn_area_size = 0;
  
  // Cached key-value pairs for DKTable
  static constexpr size_t kMaxDKTableEntries = 2000000;
  
  // Storage for keys (internal keys)
  std::vector<std::string> dktable_keys;
  // Storage for values
  std::vector<std::string> dktable_values;
  // Current index for adding entries
  uint32_t current_dktable_idx = 0;
  
  // After writing Value Area, we fill in these address info
  // Format: [offset(4 bytes), size(4 bytes)]
  std::vector<std::array<char, 8>> dktable_addr_info;
  
  // DKTable block parameters (computed from first full block)
  uint32_t dktable_block_num_entries = 0;  // entries per block
  uint32_t dktable_entry_size = 0;         // size of each entry in Learn Area
  uint32_t dktable_block_size = 0;         // size of each block in Learn Area
  uint32_t dktable_internal_key_size = 0;  // size of internal key
  
  // Learned index data for training
  std::unique_ptr<ROCKSDB_NAMESPACE::leader::LearnedIndexData> learned_index_data;
  ...
  Rep(const BlockBasedTableOptions& table_opt, const TableBuilderOptions& tbo,
      ...
      : ioptions(tbo.ioptions),
        ...
        status_ok(true),
        io_status_ok(true) {
    // Initialize DKTable vectors if learned index is enabled
    if (table_options.use_learned_index) {
      dktable_keys.reserve(1000);  // Reserve some initial capacity
      dktable_addr_info.reserve(1000);
      learned_index_data = std::make_unique<ROCKSDB_NAMESPACE::leader::LearnedIndexData>(10);
      li_build_begin_cycles = rdtscp();
    }
...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (IsValueType(value_type)) {
    ...
    // DKTable: Cache key and value for later writing
    // In Finish(), we'll write Value Area first, then Learn Area
    if (r->table_options.use_learned_index) {
      if (r->current_dktable_idx < r->kMaxDKTableEntries) {
        // Cache the full internal key and value
        r->dktable_keys.push_back(key.ToString());
        r->dktable_values.push_back(value.ToString());
        r->current_dktable_idx++;
      }
    }
...
void BlockBasedTableBuilder::WriteFooter(BlockHandle& metaindex_block_handle,
  ...
  // DKTable: Pass DKTable offsets to footer builder
  // If learned index is enabled, we have DKTable structure
  uint64_t dktable_value_area_size = 0;
  uint64_t dktable_learn_area_offset = 0;
  uint64_t dktable_learn_area_size = 0;
  
  if (r->table_options.use_learned_index && r->current_dktable_idx > 0) {
    dktable_value_area_size = r->value_area_offset;  // Size of value area
    dktable_learn_area_offset = r->learn_area_offset;
    dktable_learn_area_size = r->learn_area_size;  // Stored in Finish()
  }
  ...
  Status s = footer.Build(kBlockBasedTableMagicNumber,
                          ...
                          dktable_value_area_size, dktable_learn_area_offset,
                          dktable_learn_area_size);
  if (!s.ok()) {
    r->SetStatus(s);
    return;
  }
...
Status BlockBasedTableBuilder::Finish() {
  ...
  // ========== DKTable: Write SST with Value Area + Learn Area format ==========
  // SST Format: [Standard Data Blocks][Value Area][Learn Area][Filter][MetaIndex][Footer]
  // .li File: Only stores Leader learned index model (no key/value data)
  //
  // This follows the LeaderKV paper design where:
  //   - Value Area: all values concatenated sequentially
  //   - Learn Area: entries with [key | offset(4) | size(4)] for each KV pair
  //   - .li file: only the trained learned index model parameters
  //
  // Query flow:
  //   1. Learned index predicts position range in Learn Area
  //   2. Binary search within predicted range to find exact key
  //   3. Read value from Value Area using offset+size
  
  uint64_t dktable_value_area_offset = 0;
  uint64_t dktable_learn_area_offset = 0;
  
  if (r->table_options.use_learned_index && r->current_dktable_idx > 0) {
    std::string sst_filename = r->file->file_name();
    std::string li_filename = sst_filename + ".li";
    
    ROCKS_LOG_INFO(r->ioptions.logger,
                   "DKTable: Building DKTable format for %s with %u entries",
                   sst_filename.c_str(), r->current_dktable_idx);
    
    // ============ Step 1: Write Value Area to SST ============
    // Write all values sequentially, record their offsets
    dktable_value_area_offset = r->get_offset();
    r->dktable_addr_info.resize(r->current_dktable_idx);
    
    for (uint32_t i = 0; i < r->current_dktable_idx; ++i) {
      const std::string& value = r->dktable_values[i];
      uint32_t value_offset = static_cast<uint32_t>(r->get_offset() - dktable_value_area_offset);
      uint32_t value_size = static_cast<uint32_t>(value.size());
      
      // Store offset and size in addr_info
      std::array<char, 8> addr_buf{};
      EncodeFixed32(addr_buf.data(), value_offset);
      EncodeFixed32(addr_buf.data() + 4, value_size);
      r->dktable_addr_info[i] = addr_buf;
      
      // Write value to SST file
      IOStatus ios = r->file->Append(IOOptions(), Slice(value));
      if (!ios.ok()) {
        r->SetIOStatus(ios);
        return r->GetStatus();
      }
      r->set_offset(r->get_offset() + value_size);
    }
    
    uint64_t value_area_size = r->get_offset() - dktable_value_area_offset;
    
    // ============ Step 2: Compute DKTable block parameters ============
    if (r->current_dktable_idx > 0 && r->dktable_internal_key_size == 0) {
      r->dktable_internal_key_size = static_cast<uint32_t>(r->dktable_keys[0].size());
      // Entry in Learn Area: [key_len_varint(1-2)] [key] [offset(4)] [size(4)]
      // Simplified: assume fixed format [key(key_size)] [offset(4)] [size(4)]
      r->dktable_entry_size = r->dktable_internal_key_size + 8;  // key + addr_info(8)
      // Compute entries per 4KB block
      r->dktable_block_num_entries = 4096 / r->dktable_entry_size;
      if (r->dktable_block_num_entries == 0) r->dktable_block_num_entries = 1;
      r->dktable_block_size = r->dktable_block_num_entries * r->dktable_entry_size;
    }
    
    ROCKS_LOG_INFO(r->ioptions.logger,
                   "DKTable: Value Area offset=%lu, size=%lu bytes, entry_size=%u, entries_per_block=%u",
                   (unsigned long)dktable_value_area_offset,
                   (unsigned long)value_area_size, 
                   r->dktable_entry_size, 
                   r->dktable_block_num_entries);
    
    // ============ Step 3: Write Learn Area to SST ============
    // Each entry: [internal_key][offset(4)][size(4)]
    dktable_learn_area_offset = r->get_offset();
    
    for (uint32_t i = 0; i < r->current_dktable_idx; ++i) {
      const std::string& key = r->dktable_keys[i];
      const std::array<char, 8>& addr = r->dktable_addr_info[i];
      
      // Write key
      IOStatus ios = r->file->Append(IOOptions(), Slice(key));
      if (!ios.ok()) {
        r->SetIOStatus(ios);
        return r->GetStatus();
      }
      // Write addr_info (offset + size)
      ios = r->file->Append(IOOptions(), Slice(addr.data(), 8));
      if (!ios.ok()) {
        r->SetIOStatus(ios);
        return r->GetStatus();
      }
      r->set_offset(r->get_offset() + key.size() + 8);
    }
    
    uint64_t learn_area_size = r->get_offset() - dktable_learn_area_offset;
    
    ROCKS_LOG_INFO(r->ioptions.logger,
                   "DKTable: Learn Area offset=%lu, size=%lu bytes",
                   (unsigned long)dktable_learn_area_offset,
                   (unsigned long)learn_area_size);
    
    // ============ Step 4: Train Learned Index ============
    LocalOp learned_index_train_timer;
    LocalOp learned_index_persist_timer;
    learned_index_train_timer.begin();
    r->learned_index_data.reset(
        new leader::LearnedIndexData(r->table_options.learned_index_group_size));
    
    // Extract numeric keys for learned index (keys are already sorted in SST)
    for (uint32_t i = 0; i < r->current_dktable_idx; ++i) {
      const std::string& key = r->dktable_keys[i];
      // Extract user key (internal key minus 8-byte suffix)
      if (key.size() > 8) {
        Slice user_key(key.data(), key.size() - 8);
        uint64_t numeric_key = leader::SliceToInteger(user_key);
        r->learned_index_data->sst_keys_.push_back(numeric_key);
      }
    }
    
    // Train learned index
    bool learned_ok = false;
    if (!r->learned_index_data->sst_keys_.empty()) {
      learned_ok = r->learned_index_data->Learn();
      if (learned_ok) {
        ROCKS_LOG_INFO(r->ioptions.logger,
                       "DKTable: Learned index trained with %zu keys, min=%lu, max=%lu",
                       r->learned_index_data->sst_keys_.size(),
                       (unsigned long)r->learned_index_data->min_key_,
                       (unsigned long)r->learned_index_data->max_key_);
      }
    }
    learned_index_train_timer.record_phase();
    learned_index_train_timer.commit(kLearnedIndexTrainTime);
    
    // ============ Step 5: Write .li file (ONLY learned index model) ============
    if (learned_ok) {
      learned_index_persist_timer.begin();
      uint64_t model_size = r->learned_index_data->WriteModel(li_filename);
      ...
      learned_index_timer.phase_cycles[0] = rdtscp() - r->li_build_begin_cycles;
      learned_index_timer.max_phase_idx = 1;
      learned_index_timer.commit(kLearnedIndexBuildTime);
      
      if (model_size > 0) {
        ROCKS_LOG_INFO(r->ioptions.logger,
                       "DKTable: .li file written: %s, model_size=%lu bytes",
                       li_filename.c_str(), (unsigned long)model_size);
      } else {
        ROCKS_LOG_WARN(r->ioptions.logger,
                       "DKTable: Failed to write .li file %s", li_filename.c_str());
      }
    }
    
    // Store DKTable metadata in r for footer
    // value_area_offset stores the SIZE of value area (for footer)
    // learn_area_offset stores the OFFSET of learn area
    r->value_area_offset = value_area_size;  // SIZE of value area
    r->learn_area_offset = dktable_learn_area_offset;
    r->learn_area_size = learn_area_size;  // SIZE of learn area
    
    // Clear cached data to free memory
    r->dktable_keys.clear();
    r->dktable_values.clear();
    r->dktable_addr_info.clear();
  }
