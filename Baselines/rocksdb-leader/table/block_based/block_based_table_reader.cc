...
#include "rocksdb/leader_kv_options.h"
...
Status BlockBasedTable::Open(
  ...
  // ========== DKTable: Load offsets from Footer ==========
  // Footer stores DKTable metadata in reserved fields (format_version >= 6):
  //   - dktable_value_area_size(): Size of Value Area
  //   - dktable_learn_area_offset(): Offset of Learn Area in SST
  //   - dktable_learn_area_size(): Size of Learn Area
  // Note: Value Area offset = Learn Area offset - Value Area size
  if (footer.has_dktable()) {
    uint64_t learn_area_offset = footer.dktable_learn_area_offset();
    uint64_t value_area_size = footer.dktable_value_area_size();
    uint64_t learn_area_size = footer.dktable_learn_area_size();
    
    // Value Area is immediately before Learn Area
    rep->dktable_value_area_offset = learn_area_offset - value_area_size;
    rep->dktable_learn_area_offset = learn_area_offset;
    
    ROCKS_LOG_INFO(ioptions.logger,
                   "DKTable: Loaded from Footer - value_area_offset=%lu, "
                   "value_area_size=%lu, learn_area_offset=%lu, learn_area_size=%lu",
                   (unsigned long)rep->dktable_value_area_offset,
                   (unsigned long)value_area_size,
                   (unsigned long)rep->dktable_learn_area_offset,
                   (unsigned long)learn_area_size);
  }
  ...
  // ========== DKTable: Load learned index model from .li file ==========
  // New .li file format: ONLY contains Leader learned index model parameters
  // The .li file does NOT contain Value Area or Learn Area data
  // Key-Value data is read from the standard SST blocks
  //
  // This follows LeaderKV design where:
  //   - .li file: serialized Leader index model for fast position prediction
  //   - SST file: standard RocksDB block format (or DKTable format in full impl)
  if (s.ok() && rep->table_options.use_learned_index) {
    std::string sst_filename = rep->file->file_name();
    std::string li_filename = sst_filename + ".li";
    rep->li_file_path = li_filename;
    
    IOOptions io_opts;
    s = rep->file->PrepareIOOptions(ro, io_opts);
    if (s.ok()) {
      // Check if .li file exists
      uint64_t li_file_size = 0;
      IOStatus io_s = rep->ioptions.fs->GetFileSize(li_filename, io_opts, &li_file_size, nullptr);
      
      if (io_s.ok() && li_file_size > 0) {
        // Create LearnedIndexReader and load model from .li file
        rep->learned_index_reader.reset(new leader::LearnedIndexReader());
        Status load_status = rep->learned_index_reader->Load(li_filename);
        
        if (load_status.ok() && rep->learned_index_reader->IsLoaded()) {
          ROCKS_LOG_INFO(rep->ioptions.logger,
                         "DKTable: Learned index model loaded from %s\n"
                         "  Model file size: %lu bytes\n"
                         "  Num entries: %lu\n"
                         "  Key range: [%lu, %lu]",
                         li_filename.c_str(),
                         (unsigned long)li_file_size,
                         (unsigned long)rep->learned_index_reader->GetNumEntries(),
                         (unsigned long)rep->learned_index_reader->GetMinKey(),
                         (unsigned long)rep->learned_index_reader->GetMaxKey());
          
          // Compute dktable_entry_size and dktable_num_entries
          // If footer has DKTable info, compute from learn_area_size / num_entries
          if (rep->footer.has_dktable() && rep->learned_index_reader->GetNumEntries() > 0) {
            uint64_t learn_area_size = rep->footer.dktable_learn_area_size();
            uint64_t num_entries = rep->learned_index_reader->GetNumEntries();
            rep->dktable_num_entries = num_entries;
            rep->dktable_entry_size = static_cast<uint32_t>(learn_area_size / num_entries);
            rep->dktable_internal_key_size = rep->dktable_entry_size - 8;  // entry - offset(4) - size(4)
            
            ROCKS_LOG_INFO(rep->ioptions.logger,
                           "DKTable: entry_size=%u, internal_key_size=%u, num_entries=%lu",
                           rep->dktable_entry_size, rep->dktable_internal_key_size,
                           (unsigned long)num_entries);
          }
        } else {
          ROCKS_LOG_WARN(rep->ioptions.logger,
                         "DKTable: Failed to load learned index from %s: %s",
                         li_filename.c_str(), load_status.ToString().c_str());
          rep->learned_index_reader.reset();
        }
      } else {
        // .li file doesn't exist - that's ok, just use standard index
        ROCKS_LOG_INFO(rep->ioptions.logger,
                       "DKTable: No .li file found at %s, using standard index",
                       li_filename.c_str());
      }
    }
    // Reset status - missing .li file is not an error
    s = Status::OK();
  }
...
// ========== DKTableGet Implementation ==========
// Uses learned index to query DKTable structure in SST:
//   1. Get position prediction from learned index
//   2. Binary search in Learn Area within predicted range
//   3. Read value from Value Area using offset+size from Learn Area
bool BlockBasedTable::DKTableGet(const ReadOptions& read_options,
                                  const Slice& key,
                                  GetContext* get_context,
                                  Status* status) {
  // Check if DKTable is available
  if (!rep_->footer.has_dktable() || !rep_->learned_index_reader ||
      !rep_->learned_index_reader->IsLoaded()) {
    return false;  // Fallback to standard path
  }
  
  const Rep* r = rep_;
  
  // Extract user key from internal key (remove 8-byte suffix)
  if (key.size() <= 8) {
    return false;  // Invalid key
  }
  Slice user_key(key.data(), key.size() - 8);
  
  // Convert user key to numeric key for learned index lookup
  uint64_t numeric_key = leader::SliceToInteger(user_key);
  
  // Step 1: Get position prediction from learned index
  // GetPosition returns (lower_bound, upper_bound) range in Learn Area
  auto pos_range = r->learned_index_reader->GetPosition(user_key);
  uint64_t lower_pos = pos_range.first;
  uint64_t upper_pos = pos_range.second;
  
  // Check if key is in range (GetPosition returns (0,0) for out-of-range)
  if (lower_pos == 0 && upper_pos == 0 && r->learned_index_reader->GetNumEntries() > 0) {
    // Might be out of range or first entry - check explicitly
    uint64_t min_key = r->learned_index_reader->GetMinKey();
    uint64_t max_key = r->learned_index_reader->GetMaxKey();
    if (numeric_key < min_key || numeric_key > max_key) {
      // Key is outside the range of this SST - not found
      return true;  // "Handled" - key not in this SST
    }
  }
  
  // Step 2: Read and binary search in Learn Area
  // Learn Area format: Each entry is [internal_key (key_size bytes)][offset (4 bytes)][size (4 bytes)]
  // Entry size = internal_key_size + 8
  
  // Need to know entry size - it's stored in rep_ or can be computed
  uint32_t entry_size = r->dktable_entry_size;
  if (entry_size == 0) {
    // Entry size not initialized - need to read first entry to determine
    // For now, assume key.size() as internal key size
    entry_size = static_cast<uint32_t>(key.size()) + 8;
  }
  
  uint64_t learn_area_offset = r->dktable_learn_area_offset;
  uint64_t learn_area_size = r->footer.dktable_learn_area_size();
  uint64_t num_entries = learn_area_size / entry_size;
  
  // Clamp bounds to valid range
  if (upper_pos >= num_entries) upper_pos = num_entries - 1;
  if (lower_pos > upper_pos) lower_pos = upper_pos;
  
  // Read the range of Learn Area entries we need to search
  uint64_t read_start = learn_area_offset + lower_pos * entry_size;
  uint64_t read_size = (upper_pos - lower_pos + 1) * entry_size;
  
  // Allocate buffer for Learn Area entries
  std::unique_ptr<char[]> learn_area_buf(new char[read_size]);
  Slice learn_area_data;
  
  IOOptions io_opts;
  *status = r->file->PrepareIOOptions(read_options, io_opts);
  if (!status->ok()) {
    return false;
  }
  
  *status = r->file->Read(io_opts, read_start, read_size, &learn_area_data,
                          learn_area_buf.get(), nullptr);
  if (!status->ok()) {
    return false;
  }
  
  // Binary search in the loaded Learn Area entries
  // Compare user keys for matching (internal key = user key + 8-byte trailer)
  uint32_t internal_key_size = entry_size - 8;
  int64_t left = 0;
  int64_t right = static_cast<int64_t>(upper_pos - lower_pos);
  int64_t found_idx = -1;
  
  while (left <= right) {
    int64_t mid = left + (right - left) / 2;
    const char* entry_ptr = learn_area_buf.get() + mid * entry_size;
    
    // Compare user keys (extract user key from internal key)
    // Internal key format: [user_key][seq_num(7)][type(1)]
    Slice entry_internal_key(entry_ptr, internal_key_size);
    
    // For DKTable, we need to find exact user key match
    // Then compare by sequence number for newest version
    Slice entry_user_key = ExtractUserKey(entry_internal_key);
    Slice lookup_user_key = ExtractUserKey(key);
    
    int user_cmp = r->internal_comparator.user_comparator()->Compare(
        lookup_user_key, entry_user_key);
    
    if (user_cmp == 0) {
      // User keys match - this is our entry
      // In a sorted SST, user keys are unique (latest version only)
      found_idx = mid;
      break;
    } else if (user_cmp < 0) {
      right = mid - 1;
  ...
  if (found_idx < 0) {
    // Key not found in Learn Area
    return true;  // "Handled" - key confirmed not in this SST
  }
  
  // Step 3: Extract offset and size from Learn Area entry
  const char* found_entry = learn_area_buf.get() + found_idx * entry_size;
  const char* addr_info = found_entry + internal_key_size;
  
  uint32_t value_offset_relative = 0;
  uint32_t value_size = 0;
  memcpy(&value_offset_relative, addr_info, 4);
  memcpy(&value_size, addr_info + 4, 4);
  
  // Step 4: Read value from Value Area
  // value_offset_relative is relative to Value Area start
  // Absolute offset = dktable_value_area_offset + value_offset_relative
  uint64_t value_offset_absolute = r->dktable_value_area_offset + value_offset_relative;
  
  std::unique_ptr<char[]> value_buf(new char[value_size]);
  Slice value_data;
  
  *status = r->file->Read(io_opts, value_offset_absolute, value_size, &value_data,
                          value_buf.get(), nullptr);
  if (!status->ok()) {
    return false;
  }
  
  // Step 5: Save value to GetContext
  // Parse internal key to get sequence number and type
  ParsedInternalKey parsed_key;
  *status = ParseInternalKey(key, &parsed_key, false);
  if (!status->ok()) {
    return false;
  }
  ...
  // Save the value
  bool matched = false;
  get_context->SaveValue(parsed_key, value_data, &matched, nullptr);
  
  return true;  // Key lookup handled via DKTable path
}
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (may_match) {
    ...
    // ========== DKTable: Try learned index query first (TEMPORARILY DISABLED) ==========
    // TODO: Re-enable after fixing DKTable query implementation
    // The DKTable path currently has issues with reading Learn Area entries.
    // If DKTable structure exists in this SST (has_dktable() == true),
    // use learned index for fast position prediction + direct Value Area read.
    // This bypasses the standard index block and data block path.
#if 0
    if (rep_->footer.has_dktable() && rep_->learned_index_reader &&
        rep_->learned_index_reader->IsLoaded() && !no_io) {
      Status dktable_status;
      bool dktable_handled = DKTableGet(read_options, key, get_context, &dktable_status);
      
      if (dktable_handled) {
        // DKTable path handled the query (found or confirmed not found)
        if (!dktable_status.ok()) {
          return dktable_status;
        }
        // Check if we found the key
        if (get_context->State() == GetContext::GetState::kFound ||
            get_context->State() == GetContext::GetState::kDeleted ||
            get_context->State() == GetContext::GetState::kMerge) {
          // Key found via DKTable - done
        ...
        // DKTable said key not in this SST - we're done
        return Status::OK();
      }
      // DKTable couldn't handle it - fall through to standard path
    }
#endif
    
    // ========== Standard Index-based lookup ==========
    // Index-based lookup (with optional learned index acceleration)
    IndexBlockIter iiter_on_stack;
