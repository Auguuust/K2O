...
class BlockBasedTable : public TableReader {
 public:
  ...
  // DKTable: Query using learned index + Learn Area + Value Area
  // Returns: true if key found via DKTable path, false to fallback to standard path
  // If found, value is already saved to get_context
  bool DKTableGet(const ReadOptions& read_options, const Slice& key,
                  GetContext* get_context, Status* status);
...
struct BlockBasedTable::Rep {
  ...
  // Learned index model for fast position prediction (loaded from .li file)
  // The .li file contains ONLY the serialized learned index model parameters.
  //
  // DKTable SST Format:
  //   [Standard Data Blocks]
  //   [Value Area]     - all values concatenated (offset: dktable_value_area_offset)
  //   [Learn Area]     - entries: [key][offset(4)][size(4)] (offset: dktable_learn_area_offset)
  //   [Filter Block]
  //   [MetaIndex Block]
  //   [Footer]
  //
  // Query flow:
  //   1. learned_index->GetPosition(key) -> [lower, upper] position range
  //   2. Binary search in Learn Area within [lower, upper]
  //   3. Read value from Value Area using found offset+size
  std::unique_ptr<ROCKSDB_NAMESPACE::leader::LearnedIndexReader> learned_index_reader;
  
  // DKTable offsets in SST file
  uint64_t dktable_value_area_offset = 0;  // Start of Value Area in SST
  uint64_t dktable_learn_area_offset = 0;  // Start of Learn Area in SST
  uint64_t dktable_num_entries = 0;        // Number of entries
  uint32_t dktable_entry_size = 0;         // Size of each Learn Area entry (key_size + 8)
  uint32_t dktable_internal_key_size = 0;  // Size of internal key
  uint32_t dktable_block_num_entries = 0;  // Entries per block
  
  // Path to .li file (for debugging/logging)
  std::string li_file_path;
