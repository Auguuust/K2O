...
Status DeleteScheduler::DeleteFile(const std::string& file_path,
                                   ...
                                   const bool force_bg) {
  // DKTable: Also delete the corresponding .li file if it exists
  auto delete_li_file = [this](const std::string& sst_path) {
    if (sst_path.size() > 4 && sst_path.substr(sst_path.size() - 4) == ".sst") {
      std::string li_path = sst_path + ".li";
      Status li_s = fs_->DeleteFile(li_path, IOOptions(), nullptr);
      if (li_s.ok()) {
        ROCKS_LOG_INFO(info_log_, "Deleted .li file: %s", li_path.c_str());
      }
      // Ignore errors - .li file may not exist
    }
  };
  ...
  if (rate_bytes_per_sec_.load() <= 0 ||
    ...
    if (s.ok()) {
      delete_li_file(file_path);  // DKTable: delete .li file
  ...
  *is_complete = true;
  ...
  if (s.ok()) {
    ...
    if (need_full_delete) {
      ...
      // DKTable: Also delete the corresponding .li file
      // trash file path looks like: xxx.sst.trash, original is xxx.sst
      // so .li file is xxx.sst.li
      if (s.ok()) {
        const std::string trash_ext = ".trash";
        if (path_in_trash.size() > trash_ext.size() &&
            path_in_trash.substr(path_in_trash.size() - trash_ext.size()) == trash_ext) {
          std::string original_path = path_in_trash.substr(0, path_in_trash.size() - trash_ext.size());
          if (original_path.size() > 4 && original_path.substr(original_path.size() - 4) == ".sst") {
            std::string li_path = original_path + ".li";
            Status li_s = fs_->DeleteFile(li_path, IOOptions(), nullptr);
            if (li_s.ok()) {
              ROCKS_LOG_INFO(info_log_, "Deleted .li file from trash: %s", li_path.c_str());
            }
            // Ignore errors - .li file may not exist
          }
        }
