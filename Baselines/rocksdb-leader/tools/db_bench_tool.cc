...
// LeaderKV Learned Index Options
DEFINE_bool(use_learned_index, false,
            "Enable LeaderKV learned index for point lookups");

DEFINE_uint32(learned_index_error_bound, 32,
              "Error bound for learned index predictions");

DEFINE_uint32(learned_index_group_size, 16,
              "Number of segments grouped in second level of learned index");

// Compatibility flags for test.sh
DEFINE_string(learned_index_type, "cache",
              "cache, nocache, allinmemory (for compatibility)");
DEFINE_uint64(learned_index_cache_size_mb, 1024,
              "Learned Index Cache Size if enabled (for compatibility)");
DEFINE_bool(use_secondary_learned_index, false,
            "Use 3 Level RMI as learned index (for compatibility)");
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    if (FLAGS_use_plain_table) {
      ...
      // LeaderKV / DKTable learned index options
      block_based_options.use_learned_index = FLAGS_use_learned_index;
      block_based_options.learned_index_error_bound = FLAGS_learned_index_error_bound;
      block_based_options.learned_index_group_size = FLAGS_learned_index_group_size;
